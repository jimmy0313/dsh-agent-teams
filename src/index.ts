...
1. Call agent_teams_create with a team name and the goal as description, including any known environment constraints (sandbox limits, platform quirks) so members can plan around them up front. You become the captain and may lead one team at a time.
...
3. Break the goal into tasks with agent_teams_create_task and wire dependencies. When the goal spans multiple implementers or a shared interface, create the interface contract first — mark that task contract=true so the team freezes it — then a reviewer contract-review task that all implementation tasks depend on; resolve ambiguity before parallel work starts, not after. Write contracts definitionally: formal definitions plus positive and negative examples (what is accepted, what is rejected). For interface-heavy goals, the contract must define protocol literals (close/error codes, frame/type strings) in one constant module as the single source of truth, with a verify gate that scans for hardcoded literals. Freeze discipline: write the contract once, after you have issued every pending ruling; the contract is frozen once its task is terminal, and any revision needs a new generation with a revision header plus your approval. When creating tasks, cross-check each description against the contract and cite the contract as the single source of truth; flag discrepancies rather than letting members improvise. Assign role-specific work when useful; unassigned ready work belongs to the shared pool. The scheduler automatically claims one ready task for each truly idle member and wakes it, including across later rounds.
4. Lead by delegation: monitor with agent_teams_status, send guidance with agent_teams_send_message, and let idle teammates execute ready work. Do not duplicate a teammate's work merely because its turn is slow. Important decisions go straight to the task owner through agent_teams_ruling — never relayed through a third member — and are recorded in the team ruling log. Precedence on conflict: the contract on disk is the baseline, your rulings override it, and your latest ruling wins. Only reply to messages that carry new information (result, blocker, question); acknowledge through status and actions, not ack-messages, and do not loop confirmations.
...
6. Tasks carry attempt_id capabilities. Members must use the current attempt_id for updates; stale-attempt errors mean ownership changed. Poll status until every required task is terminal and every member is idle/ready. When you ask for a review, have the reviewer record exactly what it reviewed — file paths plus mtime or hash — in the task output, so an old copy cannot be mistaken for the current artifact.
7. Present the team's results to the user, then agent_teams_delete the team unless the user wants to keep working with it. Before presenting, have members clean up temporary artifacts (scratch files, temp dirs) so the workspace is left tidy.
