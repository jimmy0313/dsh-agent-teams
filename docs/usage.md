...
├── team.json            # 团队记录：成员、任务（含依赖）、裁定日志（rulings）、序号
└── inbox/
    ├── captain.jsonl    # 组织者邮箱（成员 → 组织者）
    └── <member>.jsonl   # 每个成员一个邮箱（JSONL）
```

任务状态机：`pending → claimed → in_progress → completed | failed | cancelled`。每次执行携带单调 `attempt` + 唯一 `attemptId`；转派先使旧 attempt 失效，再中断并等待旧成员安静，因此迟到更新无法覆盖新结果。领取前校验依赖，并禁止成员同时拥有两个未完成任务。终态任务（completed/failed/cancelled）不可变且调度器绝不重新派发。

团队可以标记**一个**任务为 `contract`（契约任务）：一旦其状态进入终态即视为契约冻结（终态不可变），修订只能以「新一代契约任务 + 修订头 + 组织者批准」进行。组织者裁定（`agent_teams_ruling`）写入 team.json 的 `rulings` 日志（`r1`、`r2`…，后者更新），直达任务 owner，优先级规则：契约以磁盘为准、裁定以队长消息为准、两者冲突时以队长最新裁定为准。
...
| `agent_teams_create_task` | 创建任务，支持 `dependencies` 依赖声明、`assignee` 指派，以及 `contract=true` 声明唯一契约任务（终态即冻结） |
...
| `agent_teams_ruling` | 组织者专属：把绑定裁定**直接**发给任务 owner 并写入团队裁定日志（`r1`…，最新裁定优先）；禁止第三方转达 |
| `agent_teams_status` | 团队全景：成员活动、任务清单（含契约标记）、裁定日志、组织者邮箱、各成员待读消息 |
...
1. **契约先行 + 契约评审前置**：多实现者 / 共享接口的目标先建「契约任务」（architect 写，`contract=true` 标记），再建「契约评审任务」（reviewer 审）作为所有实现任务的依赖——歧义在并行开工前裁决，而不是开工后返工。
...
第二次复盘（dsh-remote-mobile 中继协议工程）又收敛出五条迭代规则，已固化为插件机制 + 协议：

6. **契约单一权威 + 冻结门禁**：契约由唯一 owner 在集齐全部组织者裁定后一次性成稿；实现任务必须依赖「契约评审」任务，评审 gate 通过前调度器不会派发（`dependencies` 硬门禁）；契约任务 `contract=true` 标记、终态即冻结（不可变），修订走「新一代契约 + 修订头 + 组织者批准」。未冻结契约存在时，创建无依赖实现任务会收到提示。
7. **组织者裁定直达**：重要裁定只用 `agent_teams_ruling` 由组织者**直接**发给任务 owner，禁止第三方转达；裁定写入团队日志（`r1`、`r2`…，最新者优先）。冲突裁决规则：契约以磁盘为准、裁定以队长消息为准、两者冲突时以队长最新裁定为准。
8. **协议字面量常量化（工程规则）**：接口重型项目的契约必须把协议字面量（close/error code、frame/type 字符串）收敛到唯一常量模块作为单一事实源，实现与契约都引用常量；verify 增加「扫描禁止硬编码协议字面量」断言，杜绝一改全改、改漏一处即冲突。
9. **评审基准固定**：评审任务必须在输出中记录评审对象的文件版本（路径 + mtime/哈希），组织者给评审者明确路径与版本，避免旧副本误判新代码。
10. **停止确认循环**：任务终态即终局，调度器绝不重复派发终态任务；成员不得重复发送无新信息的确认（状态即确认）；组织者只回复「有新信息」的消息。
